...
void v_eep_read(void *data, uint16_t data_s)
{
	...
	{
		*((uint16_t *)(data + i)) = *(uint16_t *)(V_EEP_PAGE1 + i);
	}
}
...
uint8_t v_eep_write(void *data, uint16_t data_s, CRC_HandleTypeDef *hcrc)
{
	...
	const uint32_t copy_base_add = V_EEP_PAGE2 + data_s + 4;

	HAL_FLASH_Unlock();
	// Erase page 1
	FLASH_EraseInitTypeDef FLASH_EraseInitStruct = {0};
	FLASH_EraseInitStruct.TypeErase = FLASH_TYPEERASE_PAGES;
	FLASH_EraseInitStruct.PageAddress = V_EEP_PAGE1;
	...
	for (uint16_t i = 0; i < data_s; i = i + 2)
	{
		HAL_FLASH_Program(FLASH_TYPEPROGRAM_HALFWORD, (V_EEP_PAGE1 + i),
						  *(uint16_t *)(data + i));
	}
	...
	HAL_FLASH_Program(FLASH_TYPEPROGRAM_WORD, (V_EEP_PAGE1 + data_s), data_crc);

	// write copy
	FLASH_EraseInitTypeDef FLASH_EraseInitStruct = {0};
	FLASH_EraseInitStruct.TypeErase = FLASH_TYPEERASE_PAGES;
	FLASH_EraseInitStruct.PageAddress = V_EEP_PAGE2;
	FLASH_EraseInitStruct.NbPages = 1;
	uint32_t errorStatus = 0;
	HAL_FLASHEx_Erase(&FLASH_EraseInitStruct, &errorStatus);
	for (uint16_t i = 0; i < data_s; i = i + 2)
	{
		HAL_FLASH_Program(FLASH_TYPEPROGRAM_HALFWORD, (V_EEP_PAGE2 + i),
						  *(uint16_t *)(data + i));
	}
	HAL_FLASH_Program(FLASH_TYPEPROGRAM_WORD, (V_EEP_PAGE2 + data_s),
					  data_crc);
...
v_eep_read_result_t v_eep_read_verified(void *data, uint16_t data_s,
										CRC_HandleTypeDef *hcrc)
{
	uint_fast8_t is_1stOK = v_eep_verify((uint32_t *)V_EEP_PAGE1, data_s, hcrc);
	uint_fast8_t is_2ndOK = v_eep_verify((uint32_t *)V_EEP_PAGE2, data_s, hcrc);
	...
	if (!is_1stOK)
	{
		for (uint16_t i = 0; i < data_s; i = i + 2)
		{
			*((uint16_t *)(data + i)) = *(uint16_t *)(V_EEP_PAGE2 + i);
		}
